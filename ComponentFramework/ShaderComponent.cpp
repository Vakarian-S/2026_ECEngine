...
ShaderComponent::ShaderComponent(std::shared_ptr<Component> parent_, const char* vertexShaderFilename_,
    ...
    try
    {
        ...
        if (tessellationControllerFilename != nullptr && tesselationEvaluatorFilename != nullptr)
        {
            ...
            if (status == 0)
            {
                GLsizei errorLogSize = 0;
                GLsizei titleLength;
                std::string errorLog = tessellationControllerFilename;
                errorLog += "\nTESS CTRL:\n";
                titleLength = errorLog.length();
            ...
            if (status == 0)
            {
                GLsizei errorLogSize = 0;
                GLsizei titleLength;
                std::string errorLog = tesselationEvaluatorFilename;
                errorLog += "\nTESS EVAL:\n";
                titleLength = errorLog.length();
